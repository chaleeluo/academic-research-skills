...
title: Academic Pipeline — Full Academic Research Workflow Orchestrator (DAG-Enhanced)
description: "Orchestrator for the full academic research pipeline: research -> write -> integrity check -> review -> revise -> re-review -> re-revise -> final integrity check -> finalize. Coordinates deep-research, academic-paper, and academic-paper-reviewer into a seamless 10-stage workflow with mandatory integrity verification, two-stage peer review, and reproducible quality gates. DAG-enhanced: applies topological sort, critical-path analysis, and cost estimation via scripts/run_pipeline_optimizer.py. Triggers on: academic pipeline, research to paper, full paper workflow, paper pipeline, end-to-end paper, research-to-publication, complete paper workflow."
version: 3.13.0
...
metadata:
  ...
  hermes:
    tags: [Academic, Pipeline, Orchestration, Research Workflow, Automation, DAG, Optimization]
    category: pipeline
    related_skills: [deep-research, academic-paper, academic-paper-reviewer]
    requires_toolsets: [terminal, files, python3]
---

# Academic Pipeline v3.13.0 — Full Academic Research Workflow Orchestrator (DAG-Enhanced)
...
**v3.13.0 (DAG Enhancement):** Introduces optional DAG-based pipeline optimization via `scripts/run_pipeline_optimizer.py`. The orchestrator MAY run cost estimation, critical-path analysis, and incremental scheduling at key lifecycle points. All DAG output is **informational** — the orchestrator never treats machine estimates as authoritative over user preference.
...
**DAG optimization (informational, v3.13+):** After determining the entry
point, the orchestrator MAY optionally run the DAG analyzer to surface
cost, critical path, and parallelism data:

    PYTHONPATH="scripts" python3 scripts/run_pipeline_optimizer.py analyze

For mid-entry, the `incremental` action computes the remaining schedule:

    PYTHONPATH="scripts" python3 scripts/run_pipeline_optimizer.py incremental stage1 stage2

The full DAG protocol is documented in the orchestrator agent's "6. DAG-Based
...
## DAG Optimization Reference

The DAG optimizer lives at `scripts/` and is invoked by the orchestrator via the CLI:

| Action | Purpose |
|--------|---------|
| `analyze` | Full DAG topology, critical path, parallel layers |
| `plan` | Execution order with token allocation and concurrency |
| `incremental` | Remaining schedule given completed stages |
| `bottleneck` | Longest-pole stage identification |
| `cost-estimate` | Estimated USD cost and per-stage token breakdown |

Run from repo root: `PYTHONPATH="scripts" python3 scripts/run_pipeline_optimizer.py <action> [args...]`

All output is JSON, **advisory**, and never overrides user preference.
...
| Role | Full academic research workflow orchestrator (DAG-enhanced) |
