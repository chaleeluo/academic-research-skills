...
#### A0.5. Multi-Resolver Bandit-Optimized Verification — NEW v3.14

Reference: `scripts/resolver_bandit.py`, `scripts/verification_gate/__init__.py`

Before WebSearch-based verification, run the programmatic bandit-optimized resolver
pipeline via the CLI script. This replaces (and improves upon) the S2-only A0 step:
...
The script extracts citations from the paper/report, runs `verification_gate.verify_citation()`
on each, and produces a structured report. **How it works:**

1. Each citation's features (has DOI, has arXiv ID, preprint status, title length, year) are
   encoded into a feature vector.
2. The `ResolverBandit` selects the resolver order using a UCB-contextual hybrid:
   - **0.5 × UCB score** (exploitation of known success rates)
   - **0.3 × heuristic weight** (DOI → crossref preferred, arXiv → arxiv preferred)
   - **0.2 × contextual score** (per-context-key historical rewards)
3. Resolvers run sequentially in predicted-best order; search stops at first match.
4. The bandit updates its model after each citation: matched → reward 1.0, unmatched → 0.0.
5. Across 20+ citations the bandit converges: epsilon decays when gap between best/worst
   resolver exceeds 40%, and increases when all resolvers perform similarly (exploration mode).

**Routing:**

| Report Result | Action |
|--------------|--------|
| `"lookup_verified": "VERIFIED"` | Proceed to A2 (bibliographic accuracy) — skip A1 WebSearch |
| `"lookup_verified": "NOT_FOUND"` | Proceed to A1 (WebSearch existence check) as normal |
| `"lookup_verified": "UNVERIFIED"` | Proceed to A1; note that bandit exhausted all resolvers without match |
| `"error"` present | API error — skip, rely on A1 entirely |

A0.5 is additive — it does not replace A1. The audit trail must record resolver outcomes.
The bandit cache persists across runs, learning from every verification session.
