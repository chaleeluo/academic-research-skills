...
**DAG-based entry detection (informational):** After determining the entry
stage heuristically above, optionally run the DAG analyzer to confirm the
routing and surface the full dependency picture:

```
PYTHONPATH="scripts" python3 scripts/run_pipeline_optimizer.py analyze
```

For mid-entry with specific completed stages, use `incremental` to see the
remaining execution plan:

```
PYTHONPATH="scripts" python3 scripts/run_pipeline_optimizer.py incremental stage1 stage2
```
...
### 6. DAG-Based Pipeline Optimization

Run `scripts/run_pipeline_optimizer.py` at key lifecycle points to surface
cost, parallelism, and bottleneck data. The tool reads the same 12-stage
DAG defined by `scripts/pipeline_dag.py` and produces JSON output.

**At pipeline start (after intent detection):**

```
python3 scripts/run_pipeline_optimizer.py analyze
```

Shows: topological order, critical path, parallelizable layers, total token
estimate (405k), total duration estimate (325 min), stage-by-stage detail.
Present the **cost estimate** to the user during the configuration interview:

```
python3 scripts/run_pipeline_optimizer.py cost-estimate
```

Outputs estimated USD cost (≈$6 for a full run at current API rates) and a
per-stage token/duration breakdown. Include this in the Budget Transparency
presentation alongside the existing token-cost and interaction-count estimates.

**At mid-entry (user enters with existing materials):**

```
python3 scripts/run_pipeline_optimizer.py incremental <completed_stage1> [completed_stage2 ...]
```

Returns the remaining execution order, estimated duration, and suggested
concurrency. Use the output to plan the routing instead of guessing the
optimal entry point.

**At checkpoints (advisory — surface when the user asks for status):**

```
python3 scripts/run_pipeline_optimizer.py bottleneck [completed_stages...]
```

Identifies which stage is the longest pole (e.g. Stage 2 WRITE at 60 min,
Stage 4 MAJOR at 45 min) and provides a machine-generated recommendation
(intra-stage parallelism, budget reduction). Surface the bottleneck in the
"Flagged:" line of FULL checkpoints when a stage exceeds its estimated
duration.

**Execution order and parallelism (informational):**

```
python3 scripts/run_pipeline_optimizer.py plan [available_materials...]
```

Shows the optimized execution plan with parallel groups, token allocation
per stage, and savings vs sequential execution. Use this to decide HOW to
dispatch (serial vs parallel) when the pipeline has multiple ready stages.

**Rules:**
- Always `PYTHONPATH="scripts"` prefix the command, or run from the repo root.
- All output is **advisory** — the AI uses its judgment to interpret the
  JSON, never blindly trusts machine estimates over user preference.
- If `python3` is not available, fall back to `python`.
- If the script errors (no `pipeline_dag.py` in the current context), skip
  gracefully — DAG optimization is informative, not load-bearing.

### 7. ResolverBandit Citation Verification

The pipeline integrates a contextual multi-armed bandit (`scripts/resolver_bandit.py`)
that optimizes resolver selection order during integrity verification. The bandit is
triggered at **Stage 2.5** and **Stage 4.5** via the `integrity_verification_agent`.

**Invocation during integrity verification:**

When dispatching the integrity_verification_agent (Stage 2.5 or 4.5), instruct it to:

1. Export the paper's reference list to `references.json`
2. Run:
   ```
   python3 scripts/run_resolver_verification.py \
     --input references.json \
     --output resolver_report.json \
     --bandit-cache .opencode/bandit_cache.json
   ```
3. Process the resolver outcomes: VERIFIED entries skip WebSearch; NOT_FOUND/UNVERIFIED
   entries proceed to WebSearch-based A1 checking.
4. Include the resolver stats in the integrity report.

**Bandit lifecycle:**

| Phase | Action |
|-------|--------|
| First run (Stage 2.5) | Bandit starts with epsilon=0.15, uniform priors. All 4 resolvers explored. |
| After Stage 2.5 | Cache saved to `.opencode/bandit_cache.json`. Resolver success rates accumulate. |
| Stage 4.5 re-run | Cache loaded. Bandit exploits known best resolvers per context. |
| New project | Reset cache or start fresh. |

**Integration with the DAG optimizer:**

The resolver bandit is orthogonal to the DAG — it does not affect stage ordering.
However, the DAG cost estimate should account for resolver API costs:
- 4 parallel resolver calls per unbandited citation ≈ $0.002/citation
- Bandit-optimized: 1.3–2.1 calls/citation after convergence ≈ $0.0006–0.001/citation
- For a 40-reference paper: ≈$0.08 unbandited vs ≈$0.03 bandited (≈60% savings)
